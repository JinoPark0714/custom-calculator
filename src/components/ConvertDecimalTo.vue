<template>
  <div class="body">    
    ...
    <div class="input-form">
      <label class="label-custom">10진수 입력</label>
      <input v-model="decimalNumber" v-on:keyup.enter="toConvert" placeholder="너무 큰 수는 동작 안 합니다. 입력 후 엔터키"/>
    </div>
...
export default {
  ...
  methods: {
    ...
    toConvert : function(){
      const regex = /^\d+$/
      if(regex.test(this.decimalNumber)){
        const parsedNumber = parseInt(this.decimalNumber);
