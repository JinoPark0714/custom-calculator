<template>
  <div class="body">    
    ...
    <div class="input-form">
      <label class="label-custom">16진수 입력</label>
      <input v-model="hexadecimalNumber" v-on:keyup.enter="toConvert" placeholder="너무 큰 수는 동작 안 합니다. 입력 후 엔터키"/>
    </div>
    ...
    <div class="input-form">
      <label class="label-custom">8진수</label>
      <input disabled v-model="octalNumber">
    </div>
...
export default {
  ...
  methods: {
    ...
    toConvert : function(){
      ...
      if(regex.test(this.hexadecimalNumber)){
        const parsedNumber = parseInt(this.hexadecimalNumber, 16);
        this.decimalNumber = parsedNumber;
        this.binaryNumber = parsedNumber.toString(2);
        this.octalNumber = this.getOctalNumber(parsedNumber);
        return;
      }
      else
        alert("숫자를 입력해 주세요");
