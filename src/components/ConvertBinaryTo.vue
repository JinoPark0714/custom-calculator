<template>
  <div class="body">    
    ...
    <div class="input-form">
      <label class="label-custom">2진수 입력</label>
      <input v-model="binaryNumber" v-on:keyup.enter="toConvert" placeholder="너무 큰 수는 동작 안 합니다. 입력 후 엔터키"/>
    </div>
  ...
  methods: {
    ...
    toConvert : function(){
      ...
      if(regex.test(this.binaryNumber)){
        const parsedNumber = parseInt(this.binaryNumber, 2);
